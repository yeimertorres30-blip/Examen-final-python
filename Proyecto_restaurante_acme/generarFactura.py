...
def generarFactura():
    ...
    with open(ruta + "Mesas.json" , "r") as mesas:
    ...
    if datos_mesa and datos_cliente:
        ...
        while continuar == 1:
            ...
            if producto_encontrado:
                ...
                generar_archivo_ranking(producto_encontrado["Nombre"], cantidad, fecha_formateada)
...
def generarfactura(mesa, cliente, fecha, productos):
    ...
    if guardar_factura == 1:
        ...
        with open(ruta + nombre_archivo, "w") as factura:
...
def generar_archivo_para_reporte(mesa, total_productos, subtotal_bruto, subtotal_iva, subtotal, fecha, imprimio):
    ...
    try:
        with open(ruta + "respaldo_reporte.json", "r") as clientes:
                lista_datos = json.load(clientes)
    except (FileNotFoundError, json.JSONDecodeError):
        lista_datos = []
    lista_datos.append(molde_reporte)
    with open(ruta + "respaldo_reporte.json", "w") as archivo:
        json.dump(lista_datos, archivo, indent=4)
    print(decoracion)
def generar_archivo_ranking(nombre, cantidad_vendida, fecha):
    try:
    ...
    for producto in lista_productos:
        if producto["nombre_producto"] == nombre and producto["Fecha"] == fecha:
            producto["cantidad_vendida"] += cantidad_vendida
            encontrado = True
            break
    if not encontrado:    
        datos_producto = {
    ...
    with open(ruta + "ranking_productos.json", "w") as datos:
        json.dump(lista_productos, datos, indent=4)
